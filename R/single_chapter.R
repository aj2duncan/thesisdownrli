...
#'
#' The function will parse the contents of index.Rmd and find the line which
...
#' @param pdf whether to render to pdf or not, default is to render to word
#' document instead.
#'
#' @examples
#' \dontrun{
...
#' }
#' @export
single_chapter = function(chapter, pdf = FALSE) {
  ...
  # render file with chapter as child document as either
  # pdf (if selected)
  if (pdf) {
    ...
    # or word as default
    rmarkdown::render(rand_file,
                      output_file = sub("Rmd", "docx", chapter),
